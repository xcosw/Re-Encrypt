...
struct GeneratePasswordView: View {
    ...
    var body: some View {
        ...
        .onChange(of: includeLetters) { _ in generate() }
        .onChange(of: includeNumbers) { _ in generate() }
        .onChange(of: includeSymbols) { _ in generate() }
        .onChange(of: pronounceable) { _ in generate() }
        .onChange(of: usePattern) { _ in generate() }
        .onChange(of: pattern) { _ in generate() }
        .onChange(of: excludeAmbiguous) { _ in generate() }
        .onChange(of: noRepeats) { _ in generate() }
        .onChange(of: requireAllTypes) { _ in generate() }
    }
...
/*
struct GeneratePasswordView: View {
    @Binding var generatedPasswordData: Data
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Button("Generate") {
                ...
                generatedPasswordData = Data(password.utf8)
            }
