 ...
 struct PasswordManagerApp: App {
     ...
     init() {
         ...
         Task {
            await CryptoHelper.initializeSecuritySystem()
     ...
     var body: some Scene {
         WindowGroup {
             ...
             .onChange(of: appState) { oldValue, newValue in
                 Task {
                     await handleAppStateChange(from: oldValue, to: newValue)}
             }
     ...
     private func handleAppStateChange(from oldState: AppState, to newState: AppState) async {
 #if DEBUG
         ...
         case .unlocked:
             ...
             await AppInitializationHelper.initializeSecureSettings()
             await securityConfig.reload()
             ...
             Task {
                 ...
                 await MainActor.run {
                     if case .unlocked = self.appState {
                         // Only do main-actor UI work here
                     }
                 }

                 // Call async method here (no MainActor.run!)
                 await self.idleController.start()
             }
     ...
     private func handleMasterPasswordCreation(_ passwordData: Data) async  {
         ...
         let success = (try? await CryptoHelper.setMasterPassword(password)) != nil
     ...
     private func handleUnlock(with enteredData: Data) async {
         ...
         } else if await AuthenticationManager.shared.failedAttempts
                     >= AuthenticationManager.maxAttempts {
     ...
     private func handleScenePhaseChange(_ phase: ScenePhase) async {
         ...
         case .background, .inactive:
             ...
             if await CryptoHelper.getAutoLockOnBackground() {
 #if DEBUG
 ...
 #endif
             await CryptoHelper.autoLockIfNeeded()
     ...
     private func handleIdleSettingsChanged() async {
 #if DEBUG
