...
struct CoreDataHelper {
    ...
    static func savePassword(serviceName: String, username: String, lgdata: String?, countryCode: String?, phn: String, website: String, passwordData: Data, category: String, folder: Folder? = nil, context: NSManagedObjectContext) {
    ...
    static func upsertPassword(entry: PasswordEntry? = nil, serviceName: String, username: String, lgdata: String? = nil, countryCode: String?, phn: String, website: String, passwordData: Data, category: String, folder: Folder? = nil, context: NSManagedObjectContext) {
        guard CryptoHelper.validateSecurityEnvironment() else { return }
        guard let _ = CryptoHelper.keyStorage else { return }
    ...
    static func decryptedPassword(for entry: PasswordEntry) -> String? {
...
extension CoreDataHelper {
    static func createFolder(name: String, context: NSManagedObjectContext) -> Folder? {
        ...
        guard let encrypted = CryptoHelper.encryptPassword(name, salt: salt, aad: aad) else {
            context.delete(folder)
            return nil
        }
    ...
    static func renameFolder(_ folder: Folder, newName: String, context: NSManagedObjectContext) {
        ...
        guard let encrypted = CryptoHelper.encryptPassword(newName, salt: salt, aad: aad) else { return }
        folder.encryptedName = encrypted
    ...
    static func decryptedFolderName(_ folder: Folder) -> String? {
        guard let _ = CryptoHelper.keyStorage else { return nil }
        guard let encrypted = folder.encryptedName, let salt = folder.salt, let id = folder.id else { return nil }
        let aad = aadForEntry(id: id, createdAt: folder.createdAt)
        return CryptoHelper.decryptPassword(encrypted, salt: salt, aad: aad)
    }
    ...
    static func findMatchingFolder(for category: String, context: NSManagedObjectContext) -> Folder? {
        ...
        do {
            ...
            for folder in allFolders {
                if let folderName = decryptedFolderName(folder) {
    ...
    static func autoAssignEntriesToFolder(_ folder: Folder, context: NSManagedObjectContext) {
        guard let rawFolderName = decryptedFolderName(folder) else { return }
...
extension PasswordEntry {
    ...
    func getDecryptedTOTPSecret() -> String? {
        ...
        guard let decrypted = CryptoHelper.decryptPasswordData(encryptedSecret, salt: salt, aad: aad) else { return nil }
    ...
    func setEncryptedTOTPSecret(_ secret: String, context: NSManagedObjectContext) -> Bool {
        ...
        let salt = CoreDataHelper.newSalt()
        let aad = CoreDataHelper.aadForEntry(id: id, createdAt: createdAt)
        let secretData = Data(secret.utf8)
        guard let encrypted = CryptoHelper.encryptPasswordData(secretData, salt: salt, aad: aad) else { return false }
    ...
    func generateTOTPCode() -> String? {
        guard let secret = getDecryptedTOTPSecret() else { return nil }
