...
@MainActor final class CryptoHelper {
  ...
  private static func deriveKeySecurely(password: SecData, salt: Data) async throws -> SecData {
        ...
        // Convert salt Data → [UInt8] (one-time, non-secret)
    ...
    private static func saveSaltSecurely(_ salt: Data) -> Bool {
        // Derive a device-bound key for salt encryption
    ...
    private static func getDeviceID() -> Data {
        ...
        if platformExpert != 0 {
            if let cfUUID = IORegistryEntryCreateCFProperty(platformExpert, "IOPlatformUUID" as CFString, kCFAllocatorDefault, 0)?.takeUnretainedValue() as? String {
                components.append(Data(cfUUID.utf8))  // ✅ Convert String to Data
            ...
            if sysctlbyname("machdep.cpu.brand_string", &buffer, &size, nil, 0) == 0 {
                ...
                if let model = String(validating: trimmed, as: UTF8.self) {
                    if !isVirtualMachineCPU(model) {
                        components.append(Data(model.utf8))  // ✅ Convert String to Data
                        components.append(0xFF)
                    }
                }
            }
        ...
        if let mac = getPrimaryMACAddress(), !isVirtualMachineMAC(mac) {
            components.append(Data(mac.utf8))  // ✅ Convert String to Data
            components.append(0xFF)
        }
        
        // Serial number
        if let serial = getHardwareSerialNumber() {
            components.append(Data(serial.utf8))  // ✅ Convert String to Data
            components.append(0xFF)
        }
        ...
        if !username.isEmpty {
            components.append(Data(username.utf8))  // ✅ Convert String to Data
            components.append(0xFF)
        }
        ...
        if let installTime = UserDefaults.standard.string(forKey: installTimeKey) {
            components.append(Data(installTime.utf8))  // ✅ Convert String to Data
            components.append(0xFF)
        } else {
            let timestamp = "\(Date().timeIntervalSince1970)"
            UserDefaults.standard.set(timestamp, forKey: installTimeKey)
            components.append(Data(timestamp.utf8))  // ✅ Convert String to Data
            components.append(0xFF)
        }
        ...
        if let existing = UserDefaults.standard.string(forKey: deviceSaltKey) {
            components.append(Data(existing.utf8))  // ✅ Convert String to Data
            components.append(0xFF)
        } else {
            ...
            components.append(Data(deviceSalt.utf8))  // ✅ Convert String to Data
            components.append(0xFF)
        }
        ...
        if components.isEmpty {
            if let fallbackUUID = getStableFallbackUUID() {
                components.append(Data(fallbackUUID.utf8))  // ✅ Convert String to Data
                components.append(0xFF)
            } else {
                let fallback = "mac-device-\(getSystemUptime())"
                components.append(Data(fallback.utf8))  // ✅ Convert String to Data
                components.append(0xFF)
            }
        }
        
        // ✅ Hash the combined Data directly (no joining needed)
...
extension CryptoHelper {
    ...
    static func ensureDir() throws {
        ...
        var url = baseDirURL   // <-- make a mutable copy
...
extension CryptoHelper {
    ...
    static func isSessionExpired() -> Bool {
        let elapsed = getSessionElapsed()
        let timeout = getSessionTimeout()
...
extension CryptoHelper {
    ...
    static func decryptPasswordSecure(
        ...
        // Return SecData directly - it handles secure memory management
...
extension CryptoHelper {
    ...
    // MARK: - Theme Settings (Keep existing implementation)
...
final class SecurityValidator {
    ...
    static func isRunningInVM() -> Bool {
        ...
        // Trim at null terminator (required before using new initializer)
    ...
    static func validateCodeIntegrity() -> Bool {
        ...
        // Fix: Use SecCSFlags() for basic validation
        ...
        // Fix: Use SecCSFlags(rawValue: kSecCSCheckAllArchitectures)
...
/*
// MARK: - Device Binding Settings Extension
@available(macOS 15.0, *)
extension CryptoHelper {
    
    // MARK: - Device Binding Setting
    
    static func getDeviceBindingEnabled() -> Bool {
        loadSetting(key: .deviceBindingEnabled, defaultValue: true)
    }
    
    static func setDeviceBindingEnabled(_ value: Bool, context: NSManagedObjectContext) async -> Bool {
        let currentValue = getDeviceBindingEnabled()
        
        // If value hasn't changed, no need to re-encrypt
        guard currentValue != value else { return true }
        
        // Check if vault is unlocked
        guard isUnlocked else {
            secureLog("❌ Cannot change device binding: vault is locked")
            return false
        }
        
        secureLog("🔄 Device binding setting changing: \(currentValue) → \(value)")
        
        // Save the new setting first
        guard saveSetting(value, key: .deviceBindingEnabled) else {
            secureLog("❌ Failed to save device binding setting")
            return false
        }
        
        // Re-encrypt all password entries with new binding mode
        let success = await reencryptAllEntries(context: context, useDeviceBinding: value)
        
        if success {
            secureLog("✅ Device binding updated successfully")
            NotificationCenter.default.post(name: .deviceBindingChanged, object: nil)
        } else {
            secureLog("❌ Failed to re-encrypt entries with new device binding")
            // Revert the setting
            _ = saveSetting(currentValue, key: .deviceBindingEnabled)
        }
        
        return success
    }
    
    // MARK: - Re-encryption Logic
    
    private static func reencryptAllEntries(context: NSManagedObjectContext, useDeviceBinding: Bool) async -> Bool {
        let fetchRequest: NSFetchRequest<PasswordEntry> = NSFetchRequest(entityName: "PasswordEntry")
        
        do {
            let entries = try context.fetch(fetchRequest)
            secureLog("🔄 Re-encrypting \(entries.count) entries with device binding: \(useDeviceBinding)")
            
            for entry in entries {
                guard let encryptedPassword = entry.encryptedPassword,
                      let salt = entry.salt else {
                    secureLog("⚠️ Skipping entry with missing data: \(entry.serviceName ?? "unknown")")
                    continue
                }
                
                // Decrypt with current binding mode (opposite of new mode)
                let oldAAD = useDeviceBinding ? Data() : deviceBoundAAD(for: entry)
                guard let plaintext = decryptPasswordData(encryptedPassword, salt: salt, aad: oldAAD) else {
                    secureLog("❌ Failed to decrypt entry: \(entry.serviceName ?? "unknown")")
                    return false
                }
                
                // Re-encrypt with new binding mode
                let newAAD = useDeviceBinding ? deviceBoundAAD(for: entry) : Data()
                guard let newEncrypted = encryptPasswordData(plaintext, salt: salt, aad: newAAD) else {
                    secureLog("❌ Failed to re-encrypt entry: \(entry.serviceName ?? "unknown")")
                    return false
                }
                
                entry.encryptedPassword = newEncrypted
            }
            
            try context.save()
            secureLog("✅ Successfully re-encrypted all entries")
            return true
            
        } catch {
            secureLog("❌ Re-encryption failed: \(error)")
            return false
        }
    }
    
    // MARK: - Device Bound AAD Generation
    
    private static func deviceBoundAAD(for entry: PasswordEntry) -> Data {
        var aad = Data(AADEntryBindingLabel.utf8)
        aad.append(getDeviceID())
        
        // Add entry-specific binding
        if let title = entry.serviceName {
            aad.append(Data(title.utf8))
        }
        if let id = entry.id?.uuidString {
            aad.append(Data(id.utf8))
        }
        
        return aad
    }
    
    // MARK: - Encryption/Decryption with Device Binding
    
    static func encryptPasswordWithBinding(_ plaintext: String, entry: PasswordEntry) -> (encrypted: Data, salt: Data)? {
        let salt = Randomness.secureBytes(saltLength)
        let plaintextData = Data(plaintext.utf8)
        
        let aad = getDeviceBindingEnabled() ? deviceBoundAAD(for: entry) : Data()
        
        guard let encrypted = encryptPasswordData(plaintextData, salt: salt, aad: aad) else {
            return nil
        }
    ...
    static func decryptPasswordWithBinding(_ encrypted: Data, salt: Data, entry: PasswordEntry) -> String? {
        let aad = getDeviceBindingEnabled() ? deviceBoundAAD(for: entry) : Data()
        return decryptPassword(encrypted, salt: salt, aad: aad)
    }
}*/
